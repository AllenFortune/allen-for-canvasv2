...
const logStep = (step: string, details?: any) => {
  ...
  console.log(`[ADMIN-PAUSE-ACCOUNT] ${step}${detailsStr}`);
};
...
serve(async (req) => {
  ...
  try {
    logStep("Admin pause account function started");
    ...
    logStep("Pausing account for user", { userEmail, reason, adminEmail: adminUser.email });
    ...
    if (subscriber.account_status === 'paused') {
      throw new Error("Account is already paused");
    }
    ...
    if (subscriber.stripe_customer_id) {
      ...
      for (const subscription of subscriptions.data) {
        await stripe.subscriptions.update(subscription.id, {
          pause_collection: {
            behavior: 'void', // Don't charge for paused period
          },
        });
    ...
    logStep("Successfully paused account", { userEmail, adminEmail: adminUser.email });

    return new Response(JSON.stringify({ 
      success: true, 
      message: `Account for ${userEmail} has been paused` 
    }), {
    ...
    logStep("ERROR in admin-pause-account", { message: errorMessage });
